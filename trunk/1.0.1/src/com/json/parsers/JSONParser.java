...
public class JSONParser {
	...
	public Map parseJson(String content) {
		...
		Map completeJsonData=new HashMap();
	...
	private int stringLiteralTemplate(String content,int index,final char expected,final boolean flag,StringBuilder temp)
	{
		...
		if(content.charAt(index++)!=JSONConstants.DOUBLEQUOTES)
			JSONUtility.handleFailure(heirarchyList,key,"Double Quote is expected...@Position::",String.valueOf(index));
		...
		for(;;)
		{
			if(index>=maxLength)
			{
				JSONUtility.handleFailure(heirarchyList,key,"Double Quote/COMMA/"+expected+" is expected. Unexpected EOF...@Position::",String.valueOf(index));
			}	
